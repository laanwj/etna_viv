...
typedef struct _gcsSPINLOCK *           gcsSPINLOCK;
typedef struct _gcsDPContext *          gcoDPContext;
...
struct gcWhiteList
{
    gctCHAR                    processName[30];

    struct gcWhiteList *       next;
    gctINT                     throttlingFps;
};

typedef enum _gceWhiteList
{
    gcvWhiteList_Limited   = 1,
    gcvWhiteList_UnLimited = 2,
    gcvWhiteList_Customize = 3
}gceWhiteList;

struct _gcsDPContext
{
    gco3D           engine3D;
    gco2D           engine2D;
...
typedef struct _gcsPLS
{
    ...
    struct gcWhiteList *       gFpsSpecialHead;
    struct gcWhiteList *       gFpsSpecialCur;
...
gceSTATUS
gcoOS_SetDefaultDPContext(
    void
    );

gceSTATUS
gcoOS_SetGLDPContext(
    IN gcoDPContext DpContext
    );

gceSTATUS
gcoOS_ConstructDPContext(
    IN gcoOS Os,
    OUT gcoDPContext *DpContext
    );

gceSTATUS
gcoOS_DestroyDpContext(
    IN gcoOS Os,
    IN gcoDPContext DpContext
    );
...
/* Same semantics as strcspn. */
gceSTATUS
gcoOS_StrCspn(
    IN gctCONST_STRING String,
    IN gctCONST_STRING SubString,
    OUT gctSIZE_T * Output
    );
...
gceSTATUS
gcoOS_StrCat(
    IN OUT gctSTRING DestString,
    IN gctCONST_STRING SrcString
    );
...
typedef enum _gceAPIBENCH_INDEX{
    gcvAPIBENCH_INDEX_FRAME,
    gcvAPIBENCH_INDEX_DRAWARRAY,
    gcvAPIBENCH_INDEX_DRAWELEMENTS,
    gcvAPIBENCH_INDEX_BUILDSTREAM,
    gcvAPIBENCH_INDEX_INDEXBUFFER,
    gcvAPIBENCH_INDEX_DRAWARRAY_UPDATESTATE,
    gcvAPIBENCH_INDEX_DRAWARRAY_DRAWPRIMITIVE,
    gcvAPIBENCH_INDEX_DRAWELEMENT_UPDATESTATE,
    gcvAPIBENCH_INDEX_DRAWELEMENT_DRAWPRIMITIVE,
    gcvAPIBENCH_INDEX_SWAPBUFFERS,
    gcvAPIBENCH_INDEX_SETDISPLAYFLIP,
    gcvAPIBENCH_INDEX_SWTEXTUREUPLOAD,
    gcvAPIBENCH_INDEX_SWTEXTUREUPLOADSUB,
    gcvAPIBENCH_INDEX_DIRECTTEXTURE,
    gcvAPIBENCH_INDEX_GLEGLIMAGETEXTURE,
    gcvAPIBENCH_INDEX_DRAWTEX,
    gcvAPIBENCH_INDEX_BUFFERDATA,
    gcvAPIBENCH_INDEX_BUFFERSUBDATA,
    gcvAPIBENCH_INDEX_SOURCECOPY,
    gcvAPIBENCH_INDEX_HWCOPYPIXELS,
    gcvAPIBENCH_INDEX_HWSTALL,
    gcvAPIBENCH_INDEX_SETUNIFORMS,
    gcvAPIBENCH_INDEX_MAX
} gceAPIBENCH_INDEX;

#define APIBENCH_NAME {          \
    "Frame",                     \
    "DrawArray",                 \
    "DrawElement",               \
    "_BuildStream",              \
    ...
    "DrawArray_DrawPrimitive",   \
    "DrawElement_UpdateState",   \
    "DrawElement_DrawPrimitive", \
    "eglSwapBuffers",            \
    "SetDisplayFlip",            \
    "SWTextureUpload",           \
    "SWTextureUploadSub",        \
...
typedef enum _gceAPIBENCH_STATEINDEX{
    gcvAPIBENCH_STATEINDEX_FRAME,
    gcvAPIBENCH_STATEINDEX_TEXTURE,
    gcvAPIBENCH_STATEINDEX_SHADER,
    gcvAPIBENCH_STATEINDEX_MAX
}gceAPIBENCH_STATEINDEX;

#define APIBENCH_STATENAME {\
    "Frame",                \
    "Texture",              \
    "Shader",               \
}
...
typedef struct _gcsAPIBENCH_TIMER
{
    gctUINT32   start;
    gctUINT32   end;
    gctUINT32   totalTime;
    gctUINT32   count;
}gcsAPIBENCH_TIMER, *gcsAPIBENCH_TIMER_PTR;

/* global structure for api bench. */
typedef struct _gcsAPIBENCH
{
    /* Frame count. */
    gctUINT32           frameCount;

    /* Timer object for benchmark. */
    gcsAPIBENCH_TIMER   timer[gcvAPIBENCH_INDEX_MAX];

    /* Total send command size. */
    ...
    gcsAPIBENCH_TIMER   stateCounter[gcvAPIBENCH_STATEINDEX_MAX];
}gcsAPIBENCH, *gcsAPIBENCH_PTR;

#define PROFILE_DRIVER_FRAME_PRINT      10

gceSTATUS
gcoDUMP_ApiBenchStart(
    IN gcoHAL Hal,
...
gceSTATUS
gcoDUMP_ApiBenchPrint(
    IN gcoHAL Hal
    );

gceSTATUS
gcoDUMP_ApiBenchInit(
    IN gcoHAL Hal
    );

gceSTATUS
gcoDUMP_ApiBenchFrame(
    IN gcoHAL Hal
    );

gceSTATUS
gcoDUMP_ApiBenchCommand(
    IN gcoHAL Hal,
    IN gctUINT32 Size
    );

gceSTATUS
gcoDUMP_ApiBenchCommit(
    IN gcoHAL Hal
    );

gceSTATUS
gcoDUMP_ApiBenchStateStart(
    IN gcoHAL Hal,
    IN gctUINT32 StateIndex
    );

gceSTATUS
gcoDUMP_ApiBenchStateEnd(
    IN gcoHAL Hal,
    IN gctUINT32 StateIndex
    );
...
#else
#   define gcmPROFILE_INIT(freq, start)     do { } while (gcvFALSE)
...
#else
#   define gcmkPROFILE_INIT(freq, start)    do { } while (gcvFALSE)
...
#define gcoSURF_Construct(Hal, Width, Height, Depth, Type, Format, Pool, Surface) \
    _gfx_gcoSURF_Construct(Hal, Width, Height, Depth, Type, Format, Pool, Surface, __FUNCTION__, __LINE__)
...
#if MRVL_ENABLE_DUMP_SURFACE

/* dump a RGB/YUV buffer to BMP and */
int gcoOS_DumpImage(
...
/* Dump surface, support both RGB and YUV format. */
gceSTATUS
gcoSURF_DumpSurface(
    IN gcoSURF Surface,
    IN gctCONST_STRING fileName
...
#if gcmIS_DEBUG(gcdDEBUG_TRACE) || MRVL_ALWAYS_PRINT_ERROR
...
#   define gcmLOG               __dummy_log
#   define gcmkLOG              __dummy_log
#endif
...
#define _GFX_LOG_OCL_API_       0x00000200
...
#if MRVL_ENABLE_OCL_API_LOG
#define OCL_API_LOG(x)          gcoOS_Log x
#else
#define OCL_API_LOG(x)
#endif
