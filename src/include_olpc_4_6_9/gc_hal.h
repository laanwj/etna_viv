...
#define _gcmFillALVM(Bytes, Alignment, Pool, Type, Function, Line) \
{ \
    ...
    gcoOS_StrLen(Function,&funcNameLen);\
    if(funcNameLen >= gcdMAX_FUNC_NAME_LEN)\
    {\
        gcoOS_MemCopy(iface.u.AllocateLinearVideoMemory.funcName,Function,gcdMAX_FUNC_NAME_LEN-1);\
        iface.u.AllocateLinearVideoMemory.funcName[gcdMAX_FUNC_NAME_LEN-1] = '\0';\
    }\
    else\
    {\
        gcoOS_MemCopy(iface.u.AllocateLinearVideoMemory.funcName,Function,(funcNameLen+1));\
    }\
    iface.u.AllocateLinearVideoMemory.line      = Line; \
}

#define gcmFillALVM(Bytes, Alignment, Pool, Type) \
    _gcmFillALVM(Bytes, Alignment, Pool, Type, __FUNCTION__, __LINE__)
...
/* Direct read register with a physical addr*/
gceSTATUS
...
typedef enum _gceBROADCAST
{
    ...
    /* Try to Power Off GPU*/
    gcvBROADCAST_IDLE_PROFILE,
...
{
    GPUFREQ_GPU_EVENT_INIT          = 0x0,
    GPUFREQ_GPU_EVENT_DESTORY       = 0x1,
    GPUFREQ_GPU_EVENT_POWER_ON      = 0x2,
    GPUFREQ_GPU_EVENT_POWER_OFF     = 0x3
}
gceGPUFreqEvent;
...
/*
    ...
    -- gckOS_GPUFreqNotifierRegister
        register notifier block to notifier chain

    -- gckOS_GPUFreqNotifierUnregister
        un-register notifier block from notifier chain

    -- gckOS_GPUFreqNotifierCallChain
        notify power on/off event to all notifier blocks
        that was registered to notifier chain
*/
gceSTATUS
gckOS_GPUFreqNotifierRegister(
    IN gckOS Os,
...
/* Print out memory details in database */
gceSTATUS
...
/* Convert logical address to hardware specific address with passed mdl handle. */
gceSTATUS
gckHARDWARE_ConvertLogicalByHandle(
    IN gckHARDWARE Hardware,
    IN gctPOINTER Logical,
    IN gctPHYS_ADDR Handle,
    OUT gctUINT32 * Address
